...
enum DBG_PIN_REASON {
    ...
    NCM_TUD_NETWORK_RECV_FALSE,
		NCM_RECV_IRQ_PENDING,
		NCM_RECV_LARGE_PACKET,
        NCM_SENDFRAME,
	NCM_RECV_MARKER,
	NCM_DISCARDFRAME,
	NCM_SENDFRAME_QUEUE_FULL,
	NCM_RECV_QUEUE_NOT_EMPTY,

    LWIP_TCP_INPUT_LARGE_PACKET,
    LWIP_IP4_INPUT_LARGE_PACKET,
    LWIP_IP4_INPUT_LARGE_PACKET2,
    LWIP_ETHERNET_INPUT_LARGE_PACKET,
    LWIP_ETHERNET_INPUT_LARGE_PACKET2,
...
bool does_contain_citation(struct pbuf *pbuf);
...
inline constexpr uint64_t debug_chan_map[DEBUG_CHANNEL_MAP_SIZE] = {
            BIT(LWIP_MUTEX),
			BIT(ETHERNET_NETIF_INPUT),
			BIT(ETHERNET_READFRAMESIZE)|BIT(ETHERNET_READFRAMEDATA),
			BIT(CLIENT_CONNECT),
			BIT(NCM_TUD_NETWORK_RECV_CB),
			BIT(NCM_DISCARDFRAME),
			BIT(NCM_SENDFRAME_QUEUE_FULL),
			BIT(NCM_RECV_QUEUE_NOT_EMPTY),
};
