...
inline constexpr uint64_t debug_chan_map[DEBUG_CHANNEL_MAP_SIZE] = {
			BIT(CLIENT_AVAILABLE),
            BIT(CLIENT_READ),
			BIT(LWIP_TCP_INPUT_LARGE_PACKET),
			BIT(LWIP_IP4_INPUT_LARGE_PACKET2),
			BIT(LWIP_IP4_INPUT_LARGE_PACKET),
			BIT(ETHERNET_LARGE_PACKET),
            BIT(NCM_RECV_LARGE_PACKET),
